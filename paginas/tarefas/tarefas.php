  ...
  function carregarTarefas() {
    $.post('tarefas_actions.php', { action: 'get' }, function(data) {
      ...
      if (Array.isArray(data)) {
        data.forEach(item => {
          const linha = tabela.row.add([ item.tarefa_tag, item.tarefa_descricao, item.ativo_tag, `<button class="btn btn-sm btn-danger btn-apagar" data-id="${item.tarefa_codigo}" data-tag="${item.tarefa_tag}">🗑️</button>`]).draw().node();
