...
if ($action === 'get_open') {
    $sql = "WITH LatestStatus AS (
                SELECT os_tag, tag_status_id, data_inicio, ROW_NUMBER() OVER(PARTITION BY os_tag ORDER BY data_inicio DESC) as rn
                FROM SGM_OS_Status
            )
    ...
    $data = [];
    if ($stmt) { while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) { $data[] = $row; } }
    else { echo json_encode(["erro" => "Erro ao executar consulta", "details" => sqlsrv_errors()]); exit; }
...
elseif ($action === 'create') {
    ...
    $params_os = [$os_tag, $_POST['ativo_tag'], isset($_POST['maquina_parada']) ? 1 : 0, 'Corretiva', $dataCriacao, $_POST['solicitante'], $_POST['descricao_problema']];
