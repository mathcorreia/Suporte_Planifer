...
<style>
    /* Estilos para o novo layout do modal e histórico */
    .modal-body .form-section { background-color: #fff; border: 1px solid #dee2e6; border-radius: .375rem; padding: 2rem 1.5rem 1.5rem 1.5rem; margin-bottom: 2rem; position: relative; }
    .modal-body .form-section-title { position: absolute; top: -0.8rem; left: 1rem; background-color: #f8f9fa; padding: 0 0.5rem; font-size: 1rem; font-weight: 600; color: #0d6efd; }
    /* Ajustes para o Select2 ter a mesma altura dos outros campos */
    .form-control-lg-custom, .form-select-lg-custom, .select2-container .select2-selection--single { height: calc(1.5em + 1.5rem + 2px) !important; padding: 0.75rem 1rem !important; font-size: 1rem !important; border-radius: 8px !important; align-items: center; }
    .select2-container .select2-selection--single .select2-selection__rendered { line-height: 1.5 !important; padding-left: 0 !important; }
    .select2-container--bootstrap-5 .select2-selection { border: 1px solid #ced4da; }
    .history-log { list-style: none; padding-left: 0; max-height: 250px; overflow-y: auto; }
    .history-log li { background-color: #f8f9fa; border-left: 3px solid #6c757d; padding: 0.75rem; border-radius: 4px; margin-bottom: 0.5rem; }
</style>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <form id="formEdicaoOS">
                <div class="modal-header"><h5 class="modal-title"><i class="fas fa-edit me-2"></i>Seguimento OS <span id="osIdTitulo" class="badge bg-primary"></span></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body bg-light" id="edicaoOsBody"></div>
...
(function($) { 
            ...
            const formHtml = `
                ...
                <div class="form-section">
                    <h6 class="form-section-title">Novo Atendimento / Mudança de Status</h6>
                     <div class="row g-3 pt-2">
                        <div class="col-md-4"><label class="form-label">Cód. Técnico</label><input type="number" name="tecnico_codigo" class="form-control form-control-lg-custom" required></div>
                        <div class="col-md-4"><label class="form-label">Novo Status</label><select class="form-select form-select-lg-custom" name="Status" id="selectStatus">${statusOptions}</select></div>
    ...
    $('#content-area').on('click', '#btnConcluirOS', function() {
        if (!$('#formEdicaoOS input[name=tecnico_codigo]').val()) {
            alert('Por favor, informe o Código do Técnico antes de concluir.');
            return;
        }
    ...
    function initSelect2(selector, parent, action, placeholder, format) {
        $(selector).select2({
            theme: 'bootstrap-5',
            ...
            ajax: {
                ...
                processResults: function (response) {
                    return { results: $.map(response, format) };
                },
                cache: true
            }
        });
    ...
    $('#modalNovaOSS').on('shown.bs.modal', function () {
        initSelect2('select[name="ativo_tag"]', '#modalNovaOSS', 'get_ativos', 'Selecione um Ativo', item => ({ id: item.Ativo_TAG, text: item.Ativo_TAG }));
        initSelect2('select[name="solicitante"]', '#modalNovaOSS', 'get_usuarios', 'Selecione um Solicitante', item => ({ id: item.codigo, text: `${item.nome} (${item.codigo})` }));
    });
